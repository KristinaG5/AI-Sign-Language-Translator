...
class BodyMocap(object):
    def __init__(self, weights_folder, device):
        params = os.path.join(weights_folder, "body_module/data_from_spin/smpl_mean_params.npz")
        regressor_checkpoint = os.path.join(
            weights_folder,
            "body_module/pretrained_weights/smplx-03-28-46060-w_spin_mlc3d_46582-2089_2020_03_28-21_56_16.pt",
        )
        smplModelPath = os.path.join(weights_folder, "smpl/SMPLX_NEUTRAL.pkl")
        smpl_regressor = os.path.join(weights_folder, "body_module/J_regressor_extra_smplx.npy")
        self.device = device
        self.smpl = SMPLX(
            smplModelPath, regressor=smpl_regressor, batch_size=1, num_betas=10, use_pca=False, create_transl=False
        ).to(self.device)
        self.use_smplx = True

        # Load pre-trained neural network
        self.model_regressor = hmr(params).to(self.device)
        ...
        for body_bbox in body_bbox_list:
            ...
            with torch.no_grad():
                ...
                smpl_output = self.smpl(
                    betas=pred_betas, body_pose=pred_aa[:, 3:], global_orient=pred_aa[:, :3], pose2rot=True
                )
