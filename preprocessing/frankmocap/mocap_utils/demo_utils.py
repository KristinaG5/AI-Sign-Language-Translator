...
def setup_render_out(out_dir):
    if out_dir is not None:
        ...
        res_subdirs = [outputFileName, overlaidImageFolder, sideImageFolder, mergedImageFolder]
        return res_subdirs
...
def __get_input_type(args):
    ...
    if extension.lower() in video_exts:
        ...
        assert False, "Unknown input path. It should be an image," + "or an image folder, or a video file, or 'webcam' "
    return input_type
...
def save_pred_to_pkl(args, demo_type, image_path, body_bbox_list, hand_bbox_list, pred_output_list):
            ...
            if demo_type == "hand":
                ...
                for pred_key in pred_output:
                    if pred_key.find("vertices") < 0 or pred_key == "faces":
                        ...
                        if args.save_mesh:
                            if pred_key != "faces":
                                saved_pred_output[pred_key] = pred_output[pred_key].astype(np.float16)
                            else:
                                saved_pred_output[pred_key] = pred_output[pred_key]
