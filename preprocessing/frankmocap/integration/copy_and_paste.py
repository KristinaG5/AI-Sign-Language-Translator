...
def transfer_rotation(smplx_model, body_pose, part_rot, part_idx, transfer_type="g2l", result_format="rotmat"):
    ...
    part_rot_trans = __transfer_rot(body_pose_rotmat, part_rotmat, kinematic_map, transfer_type)
...
def integration_copy_paste(pred_body_list, pred_hand_list, smplx_model, image_shape):
    ...
    for i in range(len(pred_body_list)):
        ...
        if hand_info is not None and hand_info["right_hand"] is not None:
            ...
            right_hand_local_orient = transfer_rotation(smplx_model, pred_rotmat, right_hand_global_orient, 21)
        ...
        if hand_info is not None and hand_info["left_hand"] is not None:
            ...
            left_hand_local_orient = transfer_rotation(smplx_model, pred_rotmat, left_hand_global_orient, 20)
        ...
        pred_body_joints_bbox = convert_smpl_to_bbox(pred_body_joints, camScale, camTrans)
        ...
        pred_lhand_joints_bbox = convert_smpl_to_bbox(pred_lhand_joints, camScale, camTrans)
        ...
        pred_rhand_joints_bbox = convert_smpl_to_bbox(pred_rhand_joints, camScale, camTrans)
        ...
        r_hand_global_orient_body = (
            transfer_rotation(
                smplx_model, pred_rotmat, torch.from_numpy(r_hand_local_orient_body).cuda(), 21, "l2g", "aa"
            )
            .numpy()
            .reshape(1, 3)
        )  # aa
        ...
        l_hand_global_orient_body = (
            transfer_rotation(
                smplx_model, pred_rotmat, torch.from_numpy(l_hand_local_orient_body).cuda(), 20, "l2g", "aa"
            )
            .numpy()
            .reshape(1, 3)
        )
        ...
        if right_hand_local_orient is not None:
            r_hand_local_orient_hand = (
                gu.rotation_matrix_to_angle_axis(right_hand_local_orient).detach().cpu().numpy().reshape(1, 3)
            )
        ...
        if left_hand_local_orient is not None:
            l_hand_local_orient_hand = (
                gu.rotation_matrix_to_angle_axis(left_hand_local_orient).detach().cpu().numpy().reshape(1, 3)
            )
